...
namespace AQUACORE_CMPG223
{
    public partial class VisitorRestaurantOrder : System.Web.UI.Page
    {
        ...
        protected void btnPlaceOrder_Click(object sender, EventArgs e)
        {
            ...
            {
                ShowMessage("Please enter your table number.", false);
                return;
            }
            ...
            foreach (ListItem item in cblFooditems.Items)
            {
                if (item.Selected)
                {
                    ...
                    itemTotal += decimal.Parse(item.Value);
                }
            }

            if (foodItems.Length == 0)
            {
                ShowMessage("Please select at least one food item.", false);
                return;
            }

            decimal total = itemTotal * quantity;
            int visitorID = Convert.ToInt32(Session["VisitorID"]);
            string customerName = Session["VisitorName"] != null
            ...
            {
                string sql = @"INSERT INTO Visitor_Order 
                               (VisitorID, CustomerName, TableNumber, FoodItems, Quantity, TotalPrice, OrderDate, Status)
                               VALUES 
                               (@VisitorID, @CustomerName, @TableNumber, @FoodItems, @Quantity, @TotalPrice, @OrderDate, 'Pending')";
                ...
                con.Open();
                cmd.ExecuteNonQuery();
                newOrderID = con.LastInsertRowId;
            }

            ShowMessage($"Order #{newOrderID} placed successfully! Total: R{total:0.00}", true);

            // Clear the form
            txtTable.Text = "";
            txtQuantity.Text = "";
            foreach (ListItem item in cblFooditems.Items) item.Selected = false;
        }

        protected void btnMyOrders_Click(object sender, EventArgs e)
        {
            Response.Redirect("VisitorMyOrders.aspx");
        }
        ...
        {
            lblMsg.Text = message;
            lblMsg.CssClass = success ? "msg msg-success" : "msg msg-error";
        }
    }
}